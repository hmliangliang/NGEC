...
def evaluation(data,result,label):#注输入向量均为numpy.array类型
    '''此函数主要是计算各种评价指标
    data:图的邻接矩阵 data[i,j]=1表示第i个节点与第j个节点之间存在边的连接
    ...
    for i in range(data.shape[0]):
        for j in range(data.shape[0]):
            if result[0,i] == result[0,j]:#两个节点处于同一社团中
				if i!=j:#两个节点不是同一个节点
                   Q = Q + 1/(2*m)*(data[i,j] - sum(data[i,:])*sum(data[j,:])/(2*m))
    '''
    主要计算各种度量指标
    '''
    if len(label)!=0:#label为不为空
        ...
        NMI = 1/(1+np.exp(-NMI))
        return FM,K,RT,precision,recall,F1,NMI,Q#返回计算的结果
    else:
        return 0,0,0,0,0,0,0,Q
